...
public class AdminGUI extends JFrame {
    ...
    private JTextField txt_reservation_delete;
    ...
    public AdminGUI(Admin admin){
        ...
        btn_room_sh.addActionListener(e -> { // Değerlendirme Formu 13: Odaların tarihe, bölge veya otele,misafir bilgisine göre arandığı kod bloğu.
        ...
        tbl_reservation_list.getSelectionModel().addListSelectionListener(e -> {
            try{
                ...
                txt_reservation_delete.setText(select_user_id);
            }
            catch (Exception exception){
            }
        });

        /*btn_reservation_delete.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                if (Helper.isFieldEmpty(txt_reservation_delete)){
                    Helper.showMsg("fill");
                }
                else{
                    if (Helper.confirm("sure")){
                        int user_id = Integer.parseInt(txt_reservation_delete.getText());
                        if (User.delete(user_id)){
                            Helper.showMsg("done");
                            loadReservationModel();
                            txt_reservation_delete.setText(null);
                        }
                        else{
        ...
        txt_reservation_delete.setEditable(false);
        txt_reservation_delete.setFocusable(false);
        btn_reservation_delete.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                if (Helper.isFieldEmpty(txt_reservation_delete)) {
                    ...
                    if (Helper.confirm("sure")){
                        int reservation_id = Integer.parseInt(txt_reservation_delete.getText());
        ...
        txt_reservation_delete.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
    ...
    private void loadRoomListModel(ArrayList<Room> list) { // Değerlendirme Formu 14: Arama sonucunda otellerin bilgilerinin gösterildiği metod.
        DefaultTableModel clearModel = (DefaultTableModel) tbl_room_list.getModel();
        clearModel.setRowCount(0);
        int i;
        if (list == null){
